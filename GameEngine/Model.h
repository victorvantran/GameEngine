...
private:
	...
	GLuint getTextureFromFile( const char* texturePath, const std::string& directory, bool gamma = false )
	{
		//std::cout << directory + '/' + std::string( texturePath ) << std::endl;

		//std::string filepath = directory + "/" + std::string( texturePath );
...
private:
	...
	{
		// Use Assimp to load the model into a data structure of assimp, aiScene.
		// Create a scene from a file
		// Make all model's primitive shape triangles. Flip y-axis post-process
		//
		...
		this->_directory = strPath.substr( 0, strPath.find_last_of( '/' ) );
	...
	std::vector<Texture> loadMaterialTextures( aiMaterial* material, aiTextureType type, std::string typeName )
	{
		...
		for ( std::size_t i = 0; i < material->GetTextureCount( type ); i++ )
		{
			...
			{
				if ( std::strcmp( this->_textures_loaded[j].path.data(), texturePath.C_Str() ) == 0 )
				{
			...
			{
				Texture texture;
				texture.id = getTextureFromFile( texturePath.C_Str(), this->_directory );
...
public:
	Model( const char* path ) : _position( glm::vec3( 0.0f ) ), _scale( glm::vec3( 1.0f ) )
	{
		this->loadModel( path );
	}
	...
	{
		for ( std::size_t i = 0; i < this->_meshes.size(); i++ )
		{
